...
function extractTotalSystemQuota(configBody) {
    const quotaConfigData =
        configBody?.quotaConfigData && typeof configBody.quotaConfigData === "object"
            ? configBody.quotaConfigData
            : configBody;
    const quota = Number(quotaConfigData?.totalSystemQuota);
...
export default function SchoolCampaignDetail() {
                ...
                admissionMethodTimelines:
                    Array.isArray(m.admissionMethodTimelines) && m.admissionMethodTimelines.length > 0
                        ? m.admissionMethodTimelines.map((t) => ({
                              ...
                              reservationFee: t?.reservationFee != null ? String(t?.reservationFee) : "",
                              depositEndDate: String(t?.depositEndDate ?? "").slice(0, 10),
                              confirmationEndDate: String(t?.confirmationEndDate ?? "").slice(0, 10),
                          }))
                        : [{
                            methodCode: "",
                            startDate: "",
            ...
            JSON.stringify(
                (Array.isArray(formValues.admissionMethodTimelines) ? formValues.admissionMethodTimelines : []).map(
                    (t) => ({
                        ...
                        reservationFee: String(t?.reservationFee ?? "").trim(),
    ...
    useEffect(() => {
        let cancelled = false;
        getSchoolConfigByKey(SCHOOL_CONFIG_KEY.QUOTA_CONFIG_DATA)
            .then((res) => {
                if (cancelled) return;
                const body = parseSchoolConfigResponseBody(res);
                const value = extractTotalSystemQuota(body);
        ...
        if (timelines.length === 0) {
            ...
            timelines.forEach((row, idx) => {
                ...
                if (allowReservationSubmission) {
                    ...
                    if (end && confirmationEnd && confirmationEnd.getTime() > end.getTime()) {
